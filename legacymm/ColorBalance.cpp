...
bool ColorBalance::isSupported() {
    ...
    if (disp_api_supported == nullptr || !disp_api_supported(0, COLOR_BALANCE_FEATURE)) {
        return false;
    }
    ...
    if (disp_api_get_num_display_modes && disp_api_get_num_display_modes(0, 0, &count) == 0) {
        return count <= 0;
    }
...
Return<bool> ColorBalance::setColorBalance(int32_t value) {
    if (disp_api_set_color_balance != nullptr) {
        return disp_api_set_color_balance(0, static_cast<int>(value));
    }
