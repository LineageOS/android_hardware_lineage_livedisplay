...
int main() {
    ...
    int32_t (*disp_api_init)(int64_t*, uint32_t) = nullptr;
    int32_t (*disp_api_deinit)(int64_t, uint32_t) = nullptr;
    int64_t cookie = -1;
    ...
    disp_api_init =
        reinterpret_cast<int32_t (*)(int64_t*, uint32_t)>(dlsym(libHandle, "disp_api_init"));
    if (disp_api_init == nullptr) {
        LOG(ERROR) << "Can not get disp_api_init from " << SDM_DISP_LIB << " (" << dlerror() << ")";
        goto shutdown;
    }

    disp_api_deinit = (int32_t(*)(int64_t, uint32_t))dlsym(libHandle, "disp_api_deinit");
    ...
    /*
        ab = new AdaptiveBacklight(libHandle, cookie);
        if (ab == nullptr) {
    ...
    */
    ...
    /*
        if (ab->isSupported()) {
            ...
            if (status != OK) {
                LOG(ERROR) << "Could not register service for LiveDisplay HAL AdaptiveBacklight
       Iface ("
                           << status << ")";
                goto shutdown;
            }
...
shutdown:
    ...
    if (disp_api_deinit != nullptr && cookie != -1) {
